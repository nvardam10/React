    ...
    <div>
      {isLogin ?
        <div className="movieData">
          ...
          <div>
            {isAdmin && <h3 className="adde" onClick={navAdd}><u>ADD</u></h3>}
          ...
          <div className="cards">
            {
              ...
              type === "All" ?
                movieObj.slice(pid, pid + 6).map((element) => {
                  return (<MovieCard isAdmin={isAdmin} deleteDataIdx={deleteDataIdx} movieObj={element} idx={movieObj.indexOf(element)} setIdFun={setIdFun} />);
                }) : movieObj.filter((element) => {
                  // isPresentFun(()=>element.type === type)
            
                  return element.type === type;

                }).slice(pid, pid + 6).map((element) => {
          ...
          <div className="page">
            <h4>{'<'}</h4>
            <h4 style={{ margin: "20px", cursor: "pointer" }} onClick={decreaseId}><u>Pevious</u></h4>
            <h4 style={{ margin: "20px", cursor: "pointer" }} onClick={increaseId}><u>Next</u></h4>
            <h4>{'>'}</h4>
          </div>
